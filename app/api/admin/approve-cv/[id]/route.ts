...
export async function POST(
  ...
  try {
    ...
    if (userEmail) {
      try {
        const template = emailTemplates.cvApprovedNotification(userName)
        await sendEmail({
          to: userEmail,
          ...template,
        })
    ...
    return NextResponse.json({ message: 'CV approved successfully' })
  } catch (error) {
    console.error('Error:', error)
    return NextResponse.json({ message: 'Error approving CV' }, { status: 500 })
  }
}
