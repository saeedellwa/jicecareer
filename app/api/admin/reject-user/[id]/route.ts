...
export async function POST(
  ...
  try {
    ...
    try {
      const template = emailTemplates.rejectionNotification(user[0].full_name)
      await sendEmail({
        to: user[0].email,
        ...template,
      })
      ...
      await sql(
        `INSERT INTO email_logs (user_id, email_address, email_type, subject, status)
         VALUES ($1, $2, $3, $4, 'sent')`,
        [params.id, user[0].email, 'rejection', template.subject]
      )
    } catch (emailError) {
    ...
    return NextResponse.json({ message: 'Error rejecting user' }, { status: 500 })
  }
}
